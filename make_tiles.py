...
DST = Path("yolo_dataset_groupval_tiles")
...
for split in ["train", "val", "test"]:
    ...
    for img_path in sorted(src_img.glob("*.png")):
        ...
        while y0 < H:
            ...
            # Skip extremely small final tiles
            if y1 - y0 >= TILE_H * 0.5:
                ...
                for cls, x1, box_y1, x2, box_y2 in labels:
                    ...
                    original_area = max(1, (x2 - x1) * (box_y2 - box_y1))
                    intersection = iw * ih

                    # Keep a box only if at least 30% remains in this tile
                    ...
                    new_labels.append(
                        f"{int(cls)} {new_xc:.6f} {new_yc:.6f} "
                        f"{new_bw:.6f} {new_bh:.6f}"
                    )

                # Save tile if it contains at least one object
                if new_labels:
    ...
    print()
    print(split.upper())
    print("Tiled images:", total_images)
    print("Tiled boxes:", total_boxes)
